...
* CindyLinz 提到今天會有屍體可以 demo 。等等會 push [draft][0] 。

[0]: https://github.com/CindyLinz/Haskell.js/tree/draft/trans

# CindyLinz

* 本來的目標是要作 case\_reorder.hs ，發現還沒做字串（case\_reorder2.hs），會沒辦法用，所以做了簡單版。（**脫離 draft 後應補上連結**
...
* 提了一下之前的目標，也就是上次那個手工 desugar 的內容。（**脫離 draft 後應補上連結**
...
* `rhsToExp` 那邊是「亂寫」（沒有做自己的資料結構）的 XD （**應補上連結**
* 訂資料結構就像是把自己的思緒（整理得）比較清楚一點。
* 覺得麻煩的還是那個 `l` ，出來的數量不一樣。
* `AltPartial` 現在是錯的，在兩個洞以上會有問題。（**應補上連結**
* 在文件查 [`Alt l`][Alt_l] 就可以查到。
* 講解 `eatCons` 與下面使用 `AltPartial` 的例子。（**應補上連結**
* `maybeBinds` 就是那個 `where` ，在最內層要生 `rhs` 時可以一起處理掉。
* 這次的寫法像是在寫 script ，所以想介紹 Debug.Trace 這個 lib ，在 base 中就有。
  * `traceShowId`
  * `traceShow`
  * `trace`
* CindyLinz 之前寫過 [NoTrace][2] 。
...
* [SMLFamily/The-Definition-of-Standard-ML-Revised][3] 的 Atomic Patterns 。
...
[3]: https://github.com/SMLFamily/The-Definition-of-Standard-ML-Revised

# 雜談

* favonia 表示 type checking 是很崩潰的地方。CindyLinz 表示還沒做到 type checking 。
* CindyLinz 宣傳了一下 [Funth #35][Funth_35] 。出自 [Purely Functional Data Structures][4] 的 amortized queue 實作。在一般語言中，這種實作速度不穩定，但是平均下來是 O(1) 的。在 functional language 裡面要用些技巧來避免很慢的 case 。是個買保險的概念。
